...
from MacacaLibrary.locators import ElementWaiter
...
class _ElementKeywords(KeywordGroup):
    def __init__(self):
        ...
        self._element_waiter = ElementWaiter()
        self._bi = BuiltIn()
    ...
    def get_element_location(self, locator):
        ...
        """
        elements = self._element_find(locator, False, True)
        element_location = elements[0].get_property('origin').y
    ...
    def _element_input_text_by_locator(self, locator, text):
        try:
            element = self._element_find(locator, True, True)
            #logger.write("before", level='INFO', html=False)
    ...
    def _wait_element_present(self, locator, timeout):
        application = self._current_application()
        elements = self._element_waiter.wait(application, locator, timeout)
        return len(elements) > 0
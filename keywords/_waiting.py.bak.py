...
class _WaitingKeywords(KeywordGroup):
    ...
    def wait_until_page_contains_element(self, locator, time_out=5):
        ...
        #self._wait_until(timeout, error, self._wait_element_present, locator)
        timeout = int(time_out) * 1000
        self._wait_element_present(locator, timeout)
    ...
    def wait_until_page_does_not_contain_element(self, locator, timeout=None, error=None):
        ...
        def check_present():
            present = self._wait_element_present(locator)
            if not present:
                return
            else:
    ...
    def _wait_until(self, timeout, error, function, *args):
        ...
        self._wait_until_no_error(timeout, error, wait_func)

    def _wait_until_no_error(self, timeout, error, wait_func, *args):
        ...
        while True:
            ...
            if time.time() > maxtime:
                ...
                return error
            #logger.write("wait", level='INFO', html=False)
            time.sleep(0.2)

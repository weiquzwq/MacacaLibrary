import os
import robot
from macaca import WebDriver as webdriver
from MacacaLibrary.utils import BrowserCache
from MacacaLibrary import webdrivermonkeypatches
from MacacaLibrary.utils import BrowserCache
from MacacaLibrary.locators import WindowManager
...
class _BrowserManagementKeywords(KeywordGroup):
    ...
    def open_browser(self, server_url,url, alias=None, **kwargs):
        """Opens a new browser to given Macaca server.
        Capabilities of macaca server, Web,
        ...
        Examples:
        | Open Browser | http://localhost:3456/wd/hub | alias=Myapp1         | platformName=desktop           | browserName=electron|
        """
        desired_caps = kwargs
        browser = webdriver(desired_caps, str(server_url))
        browser.init()
        try:
            browser.get(url)
        except:
    ...
    def set_macaca_speed(self, seconds):
    ...
    def set_macaca_implicit_wait(self, seconds):
        """Sets macaca's default implicit wait in seconds and
